...
and again after `T-DO-DEBT-1`.
**Three tasks of ten are done**, and they are the three smallest: no
pipeline exists and nothing is deployed, so every row below except the
first is still "not started" and says so. The table is here now rather
than when there is something to report, because that is the mistake this
table exists to stop repeating.
...
set, version for version, that `T-DO-1`'s image carried.
